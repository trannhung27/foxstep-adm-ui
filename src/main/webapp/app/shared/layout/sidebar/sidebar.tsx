...
const Sidebar = (props: ISidebarProps) => {
  ...
  const findMenuItem = (arr) => {
    ...
    return Array.isArray(arr) && arr.find((item, i) => {
  ...
  const _menuProcess = (nodes) => {
    return Array.isArray(nodes) && nodes.map((item, i) => {
      if ((item.isAuthenticated && !isAuthenticated)
        ...
        || (item.title === "API" && !isOpenAPIEnabled)) return;

      const menu = _menuProcess(item.child);

      if (menu.length > 0) {
        return (
          <SubMenu key={item.key.toString()} icon={item.icon} title={item.title} style={{fontSize: '12px'}}>
            {menu}
          </SubMenu>)
      } else {
        return (
          <Menu.Item icon={item.icon} key={item.key.toString()}>
            <Link to={item.url} style={{textDecoration: 'none', fontSize: '12px'}}>{item.title}</Link>
          </Menu.Item>
        )
      }
    })
  ...
  return (
    ...
    >
      ...
      <div className="sider-trigger">
        {sidebarOpen ? <MenuFoldOutlined className="trigger" onClick={toggleSidebar}/>
